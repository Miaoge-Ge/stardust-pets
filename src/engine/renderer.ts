...
import { SCALE } from './pixel';
import { buildFrameSets } from './frames';
...
/** SR/SSR 渲染层特效:星星环绕 / 呼吸光晕 / 脚下云朵 / 移动拖尾 + 货币星星爆发 */
class EffectLayer {
  back = new Container();
  front = new Container();
  ...
  configure(effects: string[], accent: number): void {
    ...
    if (effects.includes('fx_breath')) {
      ...
      g.circle(0, 0, 62).fill({ color: accent, alpha: 0.16 });
      g.position.set(PET_X, PET_FOOT_Y - 62);
      this.breathHalo = g;
      this.back.addChild(g);
    }
    if (effects.includes('fx_cloud')) {
      const g = new Graphics();
      g.ellipse(0, 0, 34, 9).fill({ color: 0xffffff, alpha: 0.85 });
      g.ellipse(-18, 2, 14, 7).fill({ color: 0xffffff, alpha: 0.8 });
      g.ellipse(18, 2, 14, 7).fill({ color: 0xffffff, alpha: 0.8 });
      g.position.set(PET_X, PET_FOOT_Y + 6);
      this.cloud = g;
      this.back.addChild(g);
    }
    if (effects.includes('fx_orbit')) {
      for (let i = 0; i < 3; i++) {
        ...
        s.star(0, 0, 4, 6, 3).fill({ color: 0xffd66b });
  ...
  burst(count: number): void {
    for (let i = 0; i < count; i++) {
      const g = new Graphics();
      g.star(0, 0, 5, 5, 2.4).fill({ color: 0xffd66b });
      g.position.set(PET_X + (Math.random() - 0.5) * 40, PET_FOOT_Y - 130);
      const p: Particle = {
        g,
  ...
  tick(ms: number): void {
    ...
    for (let i = 0; i < this.orbitStars.length; i++) {
      ...
      s.position.set(PET_X + Math.cos(a) * 58, PET_FOOT_Y - 70 + Math.sin(a) * 26);
    ...
    if (this.cloud) {
      this.cloud.position.y = PET_FOOT_Y + 6 + Math.sin(this.orbitAngle * 1.4) * 2;
    }
    if (this.trailOn && this.moving) {
      ...
      if (this.trailAcc > 130) {
        this.trailAcc = 0;
        const g = new Graphics();
        g.star(0, 0, 4, 3.5, 1.6).fill({ color: 0xbfe3ff });
        g.position.set(PET_X + (Math.random() - 0.5) * 50, PET_FOOT_Y - 10 - Math.random() * 40);
        this.particles.push({ g, vx: (Math.random() - 0.5) * 20, vy: -14, life: 0, maxLife: 700 });
        this.front.addChild(g);
      }
    }
...
export class PetView {
  ...
  private constructor(app: Application, look: Look) {
    ...
    this.shadow.ellipse(0, 0, 36, 7).fill({ color: 0x000000, alpha: 0.18 });
    this.shadow.position.set(PET_X, PET_FOOT_Y + 5);
